...
const View = {
    ...
    els: {
        ...
        menuArt: {},   // filled in init() — screen name → its background <video>/<img>
    ...
    init() {
        ...
        document.querySelectorAll(".menu-art").forEach(el => {
            this.els.menuArt[el.id.replace("art-", "")] = el;
        });
        ...
        this.startParallax();
        this.startCursor();
        this.playScreenVideo(document.body.dataset.screen || "home");
        document.body.classList.add("loaded");
    },

    // Only the visible screen's background video actually plays — the rest
    // stay paused so we're not running 5 decoded videos at once.
    playScreenVideo(name) {
        Object.entries(this.els.menuArt).forEach(([key, el]) => {
            if (el.tagName !== "VIDEO") return;
            if (key === name) {
                const p = el.play();
                if (p && p.catch) p.catch(() => { });  // autoplay can be blocked pre-gesture — fine
            } else {
                el.pause();
            }
        });
    ...
    startClock() {
        setInterval(() => {
            this.els.clock.textContent =
                new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) + " · TX";
        }, 1000);
    },
